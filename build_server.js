...
app.post('/api/auth/login', (req, res) => {
  ...
  const user = db.prepare('SELECT id, username, role, real_name FROM users WHERE username = ? AND password = ?').get(username, hash);
...
app.get('/api/devices', authMiddleware, (req, res) => {
  ...
  const rows = db.prepare('SELECT d.*, h.hospital_name, u.real_name as engineer_name FROM devices d LEFT JOIN hospitals h ON d.hospital_id = h.id LEFT JOIN users u ON d.engineer_id = u.id ' + whereStr + ' ORDER BY d.created_at DESC LIMIT ? OFFSET ?').all(...params, parseInt(page_size), offset);
  res.json({ total, page: parseInt(page), page_size: parseInt(page_size), data: rows });
});

app.get('/api/devices/:device_code', authMiddleware, (req, res) => {
  const db = getDbSync();
  const device = db.prepare('SELECT d.*, h.hospital_name, u.real_name as engineer_name FROM devices d LEFT JOIN hospitals h ON d.hospital_id = h.id LEFT JOIN users u ON d.engineer_id = u.id WHERE d.device_code = ?').get(req.params.device_code);
...
app.get('/api/demands', authMiddleware, (req, res) => {
  ...
  const rows = db.prepare('SELECT d.*, h.hospital_name, u.real_name as creator_name FROM demands d LEFT JOIN hospitals h ON d.hospital_id = h.id LEFT JOIN users u ON d.created_by = u.id ' + whereStr + ' ORDER BY d.created_at DESC LIMIT ? OFFSET ?').all(...params, parseInt(page_size), offset);
  res.json({ total, page: parseInt(page), page_size: parseInt(page_size), data: rows });
});
...
app.post('/api/demands', authMiddleware, (req, res) => {
  ...
  db.prepare('INSERT INTO demands (demand_no,title,description,hospital_id,created_by,priority,status,created_at) VALUES (?,?,?,?,?,?,?,?)')
...
app.post('/api/demands/:demand_no/evaluate', authMiddleware, (req, res) => {
  ...
  if (eval_result === '已采纳') {
    ...
    db.prepare("UPDATE demands SET status = ?, eval_result = ?, estimated_launch_date = ?, schedule_note = ?, eval_note = ?, evaluated_by = ?, evaluated_at = datetime('now','localtime') WHERE demand_no = ?")
    ...
    db.prepare("UPDATE demands SET status = ?, eval_result = ?, reject_reason = ?, eval_note = ?, evaluated_by = ?, evaluated_at = datetime('now','localtime') WHERE demand_no = ?")
...
console.log('server.js written:', CODE.length, 'bytes');
