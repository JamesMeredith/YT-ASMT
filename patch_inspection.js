...
// Read the file
const filePath = 'E:/YT-ASMT/web/index.html';
let content = fs.readFileSync(filePath, 'utf8');

// === Part 1: Replace renderInspectionList function ===
const old1 = `    async function renderInspectionList(container) {
      container.innerHTML = \`
        <div class="card">
      ...
      try {
        ...
        wrap.innerHTML = \`
          <table class="data-table">
            <thead><tr><th>计划名称</th><th>医院</th><th>周期</th><th>下次巡检</th><th>状态</th><th>记录数</th></tr></thead>
            <tbody>
              \${r.data.map(p => \`
                <tr>
                  <td>\${p.plan_name}</td>
...
const new1 = `    async function renderInspectionList(container) {
      container.innerHTML = \`
        <div class="card">
          <div class="card-title">🔍 巡检管理</div>
          <div style="margin-bottom:16px;">
    ...
    async function loadInspectionPlans() {
      try {
        ...
        wrap.innerHTML = \`
          <table class="data-table">
            <thead><tr><th>计划名称</th><th>医院</th><th>周期</th><th>下次巡检</th><th>状态</th><th>记录数</th><th>操作</th></tr></thead>
            <tbody>
              \${r.data.map(p => {
                ...
                <tr>
                  ...
                  <td>\${getStatusTag(p.status)}</td>
                  <td>\${p.record_count||0}</td>
                  <td><button class="btn btn-sm btn-primary" onclick="openInspectionRecordModal(\${p.id},'\${safeName}')">🔍 执行巡检</button></td>
                </tr>
              \`}).join('')}
            </tbody>
          </table>
...
if (!content.includes(old1)) {
  console.error('ERROR: Could not find old renderInspectionList text');
  process.exit(1);
}
content = content.replace(old1, new1);
console.log('Part 1 replaced OK');

// === Part 2: Insert new functions before openCreateInspectionPlanModal ===
const marker = `    async function openCreateInspectionPlanModal() {`;

const newFuncs = `    async function openInspectionRecordModal(planId, planName) {
      let devices = [];
...
if (!content.includes(marker)) {
  console.error('ERROR: Could not find marker for insertion');
  process.exit(1);
}
content = content.replace(marker, newFuncs);
console.log('Part 2 inserted OK');
